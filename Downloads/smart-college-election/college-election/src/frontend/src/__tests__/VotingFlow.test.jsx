...
const recaptchaVerifierMock = vi.fn();
...
vi.mock("firebase/auth", () => ({
  RecaptchaVerifier: vi.fn(function MockRecaptchaVerifier(...args) {
    recaptchaVerifierMock(...args);
    return {
...
describe("Voting flow guards", () => {
  ...
  it("does not force Firebase test-phone mode on local development hosts", async () => {
    await openMobileLogin();

    expect(authMock.settings.appVerificationDisabledForTesting).toBe(false);
    expect(screen.queryByText(/local test mode is active/i)).toBeNull();
  });

  it("clears failed reCAPTCHA verification so sending can be retried", async () => {
    const voterSnapshot = createSnapshot({
      exists: true,
      data: {
        student_id: "22CS110",
    ...
    getDocMock.mockResolvedValue(voterSnapshot);
    signInWithPhoneNumberMock
    ...
    expect(
      await screen.findByText(/security verification failed/i),
    ).toBeTruthy();
    expect(recaptchaVerifierMock).toHaveBeenCalledWith(
      authMock,
      "sign-in-button",
    ...
    expect(recaptchaClearMock).toHaveBeenCalledTimes(1);
    expect(window.recaptchaVerifier).toBeNull();
